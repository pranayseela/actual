...
When transactions are added through the **[API](/docs/api/reference#importtransactions)** or **import/reconcile** flows, any field you set to a non-null value on the incoming row (for example category or notes) is treated as explicit input: rules run first, then those fields are copied back so they are not replaced by rule actions. Fields you leave out or set to `null` can still be set by rules. (This does not apply to institution bank-sync downloads, which are normalized from provider data and fully rule-processed.)
